...
def createConfig(configName, configDict):
    '''
    create config using the content in the configDict.
    '''
    config = open(configName, 'w')

    for nameSpace in configDict:
        line = '[' + nameSpace + ']\n'
        config.write(line)

        for key in configDict[nameSpace]:
            value = configDict[nameSpace][key]
            line = key + ' = ' + value + '\n'
            config.write(line)
...
class ConfigLoader():
    ...
    def __init__(self, colorfile, userfile, bilibilifile):
        ...
        if not os.path.exists('user.conf'):
            createConfig('user.conf', defaultUserConfig)
